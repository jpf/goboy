// ABOUTME: Tests for PPU background palette filesystem (binary format).
// ABOUTME: Validates reading and writing 66-byte CGB palette data.
...
const bgPaletteSize = 66

func setupTestPPUBGPaletteFS() (*binaryMemoryFS, *gb.Gameboy) {
	gameboy := &gb.Gameboy{}
	gameboy.CommandChan = make(chan gb.Command, 32)
	gameboy.Mu = sync.RWMutex{}
	gameboy.SetBGPalette(gb.NewPalette())

	fsys := &binaryMemoryFS{
		gb: gameboy,
		getMemory: func(gb *gb.Gameboy) []byte {
			return gb.GetBGPalette()[:]
		},
		size:        bgPaletteSize,
...
func TestPPUBGPaletteFS_Open(t *testing.T) {
	fsys, _ := setupTestPPUBGPaletteFS()
...
func TestPPUBGPaletteFile_Read(t *testing.T) {
	fsys, gameboy := setupTestPPUBGPaletteFS()

	// Set known pattern in bgpalette by directly modifying the internal structure
	gameboy.Mu.Lock()
	pal := gb.NewPalette()
	for i := 0; i < 64; i++ {
	...
	// Read entire file
	buf := make([]byte, bgPaletteSize)
	n, err := f.Read(buf)
	require.NoError(t, err)
	assert.Equal(t, bgPaletteSize, n)
...
func TestPPUBGPaletteFile_ReadAt(t *testing.T) {
	fsys, gameboy := setupTestPPUBGPaletteFS()
...
func TestPPUBGPaletteFile_ReadAt_OutOfBounds(t *testing.T) {
	fsys, _ := setupTestPPUBGPaletteFS()
...
func TestPPUBGPaletteFile_Write(t *testing.T) {
	fsys, gameboy := setupTestPPUBGPaletteFS()
	...
	case cmd := <-gameboy.CommandChan:
		assert.Equal(t, "ppu-bgpalette-write", cmd.Name)
...
func TestPPUBGPaletteFile_WriteAt(t *testing.T) {
	fsys, gameboy := setupTestPPUBGPaletteFS()
...
func TestPPUBGPaletteFile_WriteAt_OutOfBounds(t *testing.T) {
	fsys, _ := setupTestPPUBGPaletteFS()
...
func TestPPUBGPaletteFile_Stat(t *testing.T) {
	fsys, _ := setupTestPPUBGPaletteFS()
	...
	assert.Equal(t, "bgpalette", info.Name())
	assert.Equal(t, int64(bgPaletteSize), info.Size())
	assert.Equal(t, fs.FileMode(0666), info.Mode())
	assert.False(t, info.IsDir())
}

func TestPPUBGPaletteFile_ReadEOF(t *testing.T) {
	fsys, gameboy := setupTestPPUBGPaletteFS()
...
func TestPPUBGPaletteFile_ConcurrentRead(t *testing.T) {
	fsys, gameboy := setupTestPPUBGPaletteFS()
